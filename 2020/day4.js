...
const passports = readFileSync(join(__dirname, 'day4-input'), { encoding: 'utf-8' })
  ...
  .map(v => {
    const map = {};
    for (const keyValue of v.split(/\s+/).filter(v => v).map(v => v.split(':'))) {
...
for (const { byr, iyr, eyr, hgt, hcl, ecl, pid/* , cid */ } of passports) {
  if (
    // Validate birth year:
    byr && byr.length === 4 && byr >= 1920 && byr <= 2002 &&
    // Validate issue year:
    iyr && iyr.length === 4 && iyr >= 2010 && iyr <= 2020 &&
    // Validate expiration date:
    eyr && eyr.length === 4 && eyr >= 2020 && eyr <= 2030 &&
    // Validate height:
    /\d(cm|in)/.test(hgt) && (
      (hgt.includes('cm') && (() => {
