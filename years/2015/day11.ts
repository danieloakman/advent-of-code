...
function isValid(password) {
  if (FORBIDDEN_CHARS.test(password)) return false;
  ...
  if (!hasStraightOfChars) return false;
  ...
  for (const str of subStrings(password, 2)) {
    ...
    if (doubles > 1) return true;
  }
  return doubles > 1;
}

function nextChar(char) {
  return char === 'z' ? 'a' : ALPHABET[ALPHABET.indexOf(char) + 1];
}
