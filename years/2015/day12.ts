...
function numProps(object) {
  let count = 0;
  if (!Array.isArray(object) && Object.values(object).includes('red')) return count;
  for (const key in object) {
    if (typeof object[key] === 'object' && object[key] !== null) count += numProps(object[key]);
    if (typeof object[key] === 'number') count += object[key];
  }
  return count;
}
