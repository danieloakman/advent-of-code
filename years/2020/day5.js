...
function processSeat (seat) {
  ...
  for (let i = 0; i < 7; i++) {
    if (seat[i] === 'F')
      rowU -= Math.floor((rowU - rowL) / 2) + 1;
    else
      rowL += Math.floor((rowU - rowL) / 2) + 1;
  }
  let columnU = 7;
  let columnL = 0;
  for (let i = 7; i < 10; i++) {
    if (seat[i] === 'L')
      columnU -= Math.floor((columnU - columnL) / 2) + 1;
    else
      columnL += Math.floor((columnU - columnL) / 2) + 1;
  }
