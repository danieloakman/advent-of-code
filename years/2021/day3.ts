...
function powerConsumption(binaryNums) {
  const sorted = Array.from(new Array(binaryNums[0].length), _ => [0, 0]);
  binaryNums.forEach(binary => {
    for (let i = 0; i < binary.length; i++) {
      if (binary[i] === '0') sorted[i][0]++;
      else sorted[i][1]++;
    }
  });
  let gammaRate = '';
...
function lifeSupportRating(binaryNums) {
  const oxy = binaryNums.slice();
  const co2 = binaryNums.slice();
