...
function race (reindeers, seconds) {
  const distances = Object.values(reindeers)
    .reduce((map, { name }) => {
      map[name] = 0;
      return map;
    }, {});
  ...
  const getWinner = byScoring => Object.keys(byScoring)
  .sort((a, b) => byScoring[b] - byScoring[a])[0];

  for (let secs = 0; secs < seconds; secs++) {
    for (const { name, speed, fly, rest } of reindeers)
      if (secs % (fly + rest) < fly)
