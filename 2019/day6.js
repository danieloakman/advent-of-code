...
const testInput2 = once(() => ['COM)B', 'B)C', 'C)D', 'D)E', 'E)F', 'B)G', 'G)H', 'D)I', 'E)J', 'J)K', 'K)L', 'K)YOU', 'I)SAN']);
...
function countJumps (orbits, from, to) {
  ...
  while ((next = orbits[last(fromJumps)]))
    fromJumps.push(next.parent);
  const toJumps = [to];
  while ((next = orbits[last(toJumps)])) {
    if (fromJumps.includes(next.parent))
      return toJumps.length + fromJumps.indexOf(next.parent) - 2; // 2 because we don't count the last jump for each.
